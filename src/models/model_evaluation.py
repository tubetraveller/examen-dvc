...
Use:
    python model_evaluation.py --model_path models/trained_model.pkl --input_path data/processed_data_norm --metrics_path metrics/scores.json --output_path data/predictions
"""
...
def main(model_path: str, input_path: str, metrics_path: str, output_path: str):
    # Load the normalized test features from the processed_data_norm folder
    X_test_scaled = pd.read_csv(f"{input_path}/X_test_scaled.csv")
    
    # Load y_test from the original processed_data folder
    ...
    metrics = {"mse": mse_value, "r2": r2_value}
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate the trained model and save metrics/predictions."
    )
    parser.add_argument("--model_path", type=str, required=True, help="Path to the trained model.")
    parser.add_argument("--input_path", type=str, required=True, help="Path to X_test_scaled (from data/processed_data_norm).")
